    ...
    <div class="form-group">
        ...
        <div class="col-sm-12">
            <input
                ...
                @if(isset($object) && $object->data[$f['name']])
                {!! (array_key_exists('disabled_on_edit',$f) && $f['disabled_on_edit'] == 'yes') ? 'readonly="readonly"' : '' !!}
                @endif
                data-rule-slug="true"
                ...
                @if(is_array($f['message_error']) && array_key_exists('slug',$f['message_error'])) data-msg-slug="{{ ucfirst(trans_custom('backend/content.'.$type.'.field_container.'.$index.'.field.'.$i.'.message_error.slug', $f['message_error']['slug'])) }}" @endif
                @if(array_key_exists('placeholder',$f)) placeholder="{{ trans_custom('backend/content.'.$type.'.field_container.'.$index.'.field.'.$i.'.placeholder', $f['placeholder']) }}" @endif

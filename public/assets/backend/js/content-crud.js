$(document).ready(function() {
    ...
    $.validator.addMethod("phone", function(value, element) {
        var phone = value.replace(/ |_/g, '');
        if (phone.length > 5 || phone.length <= 1) return true;
        return false;
    }, "Please input valid phone number");
    $.validator.addMethod("creditcard", function(value, element) {
        var cc = value.replace(/-|_/g, '');
        if (cc.length == 16 || cc.length == 0) return true;
        return false;
    ...
    var validator = $(".form-validate-jquery").validate({
        ...
        errorPlacement: function(error, element) {
            ...
            // Select option
            else if (element.siblings().hasClass('bootstrap-select')) {
        ...
        showErrors: function(errorMap, errorList) {
            hide_container_error();
            if (this.numberOfInvalids() > 0) {
                $(errorList).each(function() {
                    show_container_error(this.element);
                });
            }
    ...
    $(".file-styled-primary").uniform({
        wrapperClass: 'bg-success-400',
        fileButtonHtml: '<i class="icon-file-picture"></i>'
    });
    ...
    if (Array.prototype.forEach) {
        ...
        elems.forEach(function(html) {
            if ($(html).hasClass('switchery-primary')) var switchery = new Switchery(html, { color: '#2196F3' });
            else if ($(html).hasClass('switchery-danger')) var switchery = new Switchery(html, { color: '#EF5350' });
            else if ($(html).hasClass('switchery-warning')) var switchery = new Switchery(html, { color: '#FF7043' });
            else if ($(html).hasClass('switchery-info')) var switchery = new Switchery(html, { color: '#00BCD4' });
            else var switchery = new Switchery(html);
        });
    }
    ...
    $(".publish-radio").uniform({
        radioClass: 'choice'
    });

    $('#publish_status').on('change', function() {
        $('#c_publish_status').val($(this).val());
    });

    $('#content_product_selector').on('change', function () {
        $('#product_id').val($(this).val());
    });

    $('#customize').on('change', function() {
        $('#c_customize').val($(this).val());
    });

    $('#attr_set_id').on('change', function() {
        $('#c_attridset').val($(this).val());
    });

    $('input:radio[name="categoryproduct"]').change(function() {
        $('#c_categoryID').val($(this).val());
    });

    $('input:radio[name="layouttemplate"]').change(function() {
        $('#c_layouttemplate').val($(this).val());
    });

    $('input:radio[name="categorygallery"]').change(function() {
        $('#c_categorygalleryID').val($(this).val());
    });


    $('input:radio[name="changeimage"]').change(function() {
        // $('#c_categoryID').val($(this).val());
        if($(this).val() == 1)
        {
            $('.tabbable').slideDown();
    ...
    $('#content_publish_time_container .publish-time-action-button.button-ok').on('click', function() {
        if ($('#radio_publish_default').prop('checked') == true) {
            ...
            $('#c_publish_time').val(moment($('#publish_time_picker').val()).format('YYYY-MM-DD HH:mm:ss'));
    ...
    $('#content_template_selector').on('change', function() {
        $('#c_template').val($(this).val());
    });

    $('#content_order_field').on('change', function() {
        $('#c_order').val($(this).val());
    });
    ...
    $('#button_delete').on('click', function() {
        ...
        bootbox.confirm({
            ...
            callback: function(result) {
                if (result) {
                    window.location.href = $('#button_delete').attr('data-action');
                }
            }
        });
    });
    ...
    $('#button_preview').on('click', function() {
        // window.location.href = $(this).attr('data-action');
        var win = window.open($(this).attr('data-action'), '_blank');
        win.focus();
    });

    function collectFeaturedImage() {
        if ($('#featured_image_field').length) {
    ...
    function collectImageGallery() {
        if ($('.images-gallery-field-body .images-galley-thumb-container').length) {
            var imageGallery = [];
    ...
    function collectFeaturedVideo() {
        if ($('#featured_video_internal').length) {
    ...
    function collectTemplateValue() {
        if ($('#content_template_selector').length) {
            $('#c_template').val($('#content_template_selector').val());
        }
    }
    ...
    function show_container_error(element) {
        var containerId = '';
        containerId = $(element).attr('data-container');
        console.log('show error ' + containerId);
    }

    function hide_container_error() {
        console.log('hide container error ');
    }

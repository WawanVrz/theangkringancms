...
! function(a) {
    ...
    ! function() {
        ...
        a.validator.addMethod("password", function(value, element) {
            return result = this.optional(element) || value.length >= 6 && /\d/.test(value) && /[a-z]/i.test(value);
        }, "Password must be at least 6 characters and contain letter and numeric (A-Z,0-9)"),
        a.validator.addMethod("password_strong", function(value, element) {
            return result = this.optional(element) || value.length >= 6 && /\d/.test(value) && /[a-z]/i.test(value) && /[!#$&()*+<=>@\[\]^]/i.test(value);
        }, "Password must be at least 6 characters and contain letter, numeric, & symbol (A-Z,0-9,!#$&()*+<=>@[]^)"),
        a.validator.addMethod("slug", function(value, element) {
            return result = this.optional(element) || /^[a-z0-9]+(?:[-/][a-z0-9]+)*$/.test(value);
        }, "Please use valid slug characters: a-z, 0-9, /, or -")
});
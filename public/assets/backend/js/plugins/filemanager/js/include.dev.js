...
! function(e, a, t) {
    ...
    function selectItems(a) {
        ...
        var o = e("#base_url").val(),
            ...
            c = encodeURL((1 == l ? r : o + i) + e(a).attr('data-file'));
        var fileBox = e(a).closest('.file');

        if (e(fileBox).hasClass('item-selected')) {
            for (var i = 0; i < selected_items.length; i++) {
                if (selected_items[i].url == c) {
            ...
            var file = {};
            file.filename = e(a).attr('data-file');
            file.dimension = e(a).siblings('.img-dimension').html();
            file.extension = e(a).siblings('.file-extension').html();
            file.size = e(a).siblings('.file-size').html();
            file.date = e(a).siblings('.file-date').html();
            file.url = c;
            selected_items.push(file);
        }
        e('.select-files-button').removeClass('button-active');
        if (selected_items.length > 0) e('.select-files-button').addClass('button-active');
        e(fileBox).toggleClass('item-selected');
        return false;
    }

    e(document).ready(function() {
        e('.select-files-button').on('click', function() {
            if (selected_items.length < 1) return false;
            else {
                var c = JSON.stringify(selected_items);
                // alert(c);
                var t = e("#field_id").val()
        ...
        window.resizeTo(e(window).width(), e(window).height());
    });
    ...
    var j = "9.9.6",
        ...
        U = {
            ...
            bindGridEvents: function() {
                function a(a) {
                    if (e('#multi_select').val() == '1') return selectItems(a);

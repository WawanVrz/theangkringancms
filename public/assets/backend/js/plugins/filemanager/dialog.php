...
$multiselect = 0;
if(array_key_exists('multiSelect',$_GET)) $multiselect = $_GET['multiSelect'];
...
<html xmlns="https://www.w3.org/1999/xhtml">
	<head>
		...
		<link href="css/style.css?<?php echo uniqid(); ?>" rel="stylesheet" type="text/css" />
	...
	{
	?>
		<script src="js/aviary/editor.js"></script>
	<?php
	}
	...
	<script src="js/include.dev.js?<?php echo uniqid(); ?>"></script>
</head>
<body>
	...
	<input type="hidden" id="multi_select" value="<?php echo $multiselect; ?>" />
<?php if($upload_files){ ?>
<!-- uploader div start -->
<div class="uploader">
...
foreach($files as $k=>$file){
    ...
    elseif(is_dir($current_path.$rfm_subfolder.$subdir.$file)){
		...
		if(substr($file, 0, 1) != '.')
		{
			if($_GET['type'] == 1) 
			{
				if(in_array($file_ext,$ext_img)) $current_files_number++;
			}
			else if($_GET['type'] == 3) 
			{
				if(in_array($file_ext,$ext_video)) $current_files_number++;
			}
			else $current_files_number++;
		}
    }
}
    ...
    <div class="navbar-inner">
		...
		<div class="filters">
			...
			<div class="span6 entire types">
				...
				<label id="ff-item-type-all" title="<?php echo trans('All'); ?>" <?php if($_GET['type']==1 || $_GET['type']==3){ ?>style="visibility: visible;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><i class="icon-repeat icon-white" style="position:relative; top:2px;"></i></label>
			</div>
		    </div>
		</div>
	    </div>
...
<!-- header div end -->
	...
	<li class="pull-right">
		<button class="tip btn select-files-button">Select Files</button>
	</li>
	<?php endif; ?>
